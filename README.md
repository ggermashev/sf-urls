...
При запуске [приложения](./src/Client.java) в терминале появляется меню. 
...
Реализовано в [FileManager](./src/utils/FileManager.java).getEnv() 
и [UrlController](./src/Controllers/UrlController.java).createShortUrl()
...
сгенерирован на серверной части приложения. По этому токену доступа происходит идентификация при повторных запросах. 
При выходе из аккаунта, токен доступа "протухает". Стоит отметить, что
